...
2. the **welfare side**, where the induced social welfare function lands in
   a `PrefOrder`-based framework.

This avoids trying to force Nipkow's strict-order proof through weak orders.
-/
...
lemma switchProfile_succ
  ...
  · subst hj
    ...
    have hi_not_lt : ¬ ((e i : ℕ) < k) := by
      rw [hi_eq]
      exact Nat.not_lt.mpr (Nat.le_refl k)
...
This is Nipkow's monotonicity lemma.
-/
lemma monotonicity
    (g : SCF ι σ) (X : Finset σ)
...
section SWFConstruction

variable {ι σ : Type*}
variable [Fintype ι] [DecidableEq ι] [DecidableEq σ]

/--
The binary-top profile, viewed on the weak-order side.
-/
noncomputable def weakTopProfile (S : Finset σ) (P : SProfile ι σ) : Profile ι σ :=
  weakProfileOf (topProfile S P)
...
abbrev SSWF (ι σ : Type*) := SProfile ι σ → PrefOrder σ

end SWFConstruction

section TopUnanimityAndArrowPremises
